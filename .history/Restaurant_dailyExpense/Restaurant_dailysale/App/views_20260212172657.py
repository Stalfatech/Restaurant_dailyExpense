...
def add_expense(request):
    user = request.user  # safer than session login_id

    # Check if day is closed
    if CashBook.objects.filter(date=date.today(), branch=user.branch, is_closed=True).exists():
    ...
    if form.is_valid():
        ...
        if user.user_type == 0:  # Admin
            # branch comes from form.cleaned_data['branch']
