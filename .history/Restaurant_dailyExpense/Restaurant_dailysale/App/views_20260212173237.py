...
def add_expense(request):
     # use request.user instead of session

    # Check if day is closed
    if CashBook.objects.filter(date=date.today(), branch=request.user.branch, is_closed=True).exists():
    ...
    return render(request, 'expenses/expense_form.html', {'form': form, 'user_type': user_type})
