...
def dashboard_expenses(request):
    ...
    if user.user_type == 0:  # Admin sees all branches
        expenses = Expense.objects.filter(date=selected_date, status='Approved')
    else:  # Manager sees only their branch
        expenses = Expense.objects.filter(date=selected_date, status='Approved', branch=user.branch)
...
def edit_expense(request, pk):
    user = request.user  # always a logged-in user

    # Fetch expense based on role
    if user.user_type == 0:  # Admin can edit any
        expense = get_object_or_404(Expense, pk=pk)
    else:  # Manager can edit only for their branch
