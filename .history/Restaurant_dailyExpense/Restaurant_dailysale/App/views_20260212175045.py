...
def dashboard_expenses(request):
    # Get selected date from query params (string) and convert to date
    ...
    if user.user_type == 0:  # Admin
        expenses = Expense.objects.filter(date=selected_date)  # all statuses
    else:  # Manager
        expenses = Expense.objects.filter(date=selected_date, branch=user.branch)  # all statuses
