...
def dashboard_expenses(request):
    ...
    if user.user_type == 0:  # Admin: see all branches
        # If date is DateField
        expenses = Expense.objects.filter(date=selected_date)
        # If date is DateTimeField:
        # expenses = Expense.objects.filter(date__date=selected_date)
    else:  # Manager: only their branch
        expenses = Expense.objects.filter(date=selected_date, branch=user.branch)
        # If date is DateTimeField:
        # expenses = Expense.objects.filter(date__date=selected_date, branch=user.branch)
...
def add_expense(request):
    ...
    if form.is_valid():
        ...
        if user.user_type == 0:  # Admin
            ...
            # Status can be set by Admin or default to Approved
            expense.status = form.cleaned_data.get('status') or 'Approved'
        else:  # Manager
            expense.branch = user.branch
            expense.status = 'Pending'  # Managers cannot approve
