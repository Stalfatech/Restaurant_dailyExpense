...
def add_expense(request):
    ...
    # Check if day is closed for this user's branch
    ...
    if form.is_valid():
        ...
        if user.user_type == 0:  # Admin
            # Admin can pick branch and status
            branch = form.cleaned_data.get('branch')
            status = form.cleaned_data.get('status') or 'Approved'
            if not branch:
            ...
            # Manager always uses their branch and Pending status
            if not user.branch:
