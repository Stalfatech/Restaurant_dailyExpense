...
class DeliverySaleForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not platform:
            self.add_error('platform', 'This field is required because amount is filled.')
        if amount is None:
            self.add_error('amount', 'This field is required because platform is filled.')
        elif amount <= 0:
...
class DailySaleItemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'meal_type': forms.Select(attrs={'class': 'form-select meal-field'}),
            'item_name': forms.TextInput(attrs={'class': 'form-control meal-field', 'placeholder': 'Enter item name'}),
            'amount': forms.NumberInput(attrs={'class': 'form-control meal-field', 'step': '0.01'}),
        }

    def clean(self):
        cleaned_data = super().clean()
        meal = cleaned_data.get('meal_type')
        item = cleaned_data.get('item_name')
        amount = cleaned_data.get('amount')

        # Ignore completely empty row
        if not meal and not item and not amount:
            return cleaned_data

        # If any value filled, all must be filled
        if not meal:
            self.add_error('meal_type', 'This field is required because other fields in this row are filled.')
        if not item:
            self.add_error('item_name', 'This field is required because other fields in this row are filled.')
        if amount is None:
            self.add_error('amount', 'This field is required because other fields in this row are filled.')
        elif amount <= 0:
            self.add_error('amount', 'Amount must be greater than 0.')
