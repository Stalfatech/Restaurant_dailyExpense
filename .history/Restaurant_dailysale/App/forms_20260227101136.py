...
from django.core.exceptions import ValidationError
import re
# Assuming your model is imported here, e.g.:
# from .models import CommunicationSettings
...
class CommunicationSettingsForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            # Use TextInput to allow '+' and spaces
    ...
    def clean_whatsapp_number(self):
        """
        Validates that the WhatsApp number starts with + and contains digits.
        Regex matches: Optional +, then country code and number (6-15 digits total).
        """
        whatsapp = self.cleaned_data.get('whatsapp_number')
        
        if not whatsapp:
            return whatsapp # Allow empty if it's not required

        # Remove spaces, hyphens, or brackets for validation
        cleaned_phone = re.sub(r'[\s\-\(\)]', '', whatsapp)

        # Regex: Starts with +, followed by 7 to 15 digits (covers int'l + 10 digits)
