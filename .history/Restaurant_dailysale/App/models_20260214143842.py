...
class DailySale(models.Model):
    ...
    branch = models.ForeignKey(Branch, on_delete=models.CASCADE,def)
    date = models.DateField(
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True)
    ...
    def calculate_totals(self):
            dining_total = self.breakfast + self.lunch + self.dinner + self.delivery

        # Total Sales
            self.total_sales = dining_total + self.pos_amount

        # Cash Sales Logic
            if self.pos_type == "Cash":
             self.cash_sales = dining_total + self.pos_amount
            else:
             self.cash_sales = dining_total
