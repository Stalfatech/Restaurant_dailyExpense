...
def send_email_report(client_email, subject, body, pdf_file=None):
    ...
    if not config:
        return "No active Communication Settings found"
...
def notify(request, text, tag="success"):
    # Determine user name and branch
    name = getattr(request.user, 'name', '') or "Admin"
    branch = getattr(request.user, 'branch', None) if request.user.user_type in [1,2] else None
