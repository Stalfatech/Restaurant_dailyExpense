...
from .utils.google_vision_parser import extract_invoice_data
from django.views.decorators.csrf import csrf_exempt
import tempfile
import os
    
def extract_invoice_data_ajax(request):
    ...
    if form.is_valid():
        try:
            with transaction.atomic():
                ...
                if expense.invoice:
                    try:
                        extracted_amount = extract_invoice_data(expense.invoice.path)
                        if extracted_amount and 'grand_total' in extracted_amount:
                            expense.amount = extracted_amount['grand_total']
