...
import tempfile, os
from .utils.google_vision_parserparser import extract_amount_from_invoice
...
def extract_invoice_amount_ajax(request):
    if request.method == "POST" and request.FILES.get("invoice"):
        ...
        try:
            amount_line = extract_amount_from_invoice(temp_path)
        finally:
            os.remove(temp_path)

        return JsonResponse({
            "amount": amount_line if amount_line else ""
        })
