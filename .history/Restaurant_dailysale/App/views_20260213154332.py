...
import tempfile, os
from .utils.google_vision_parser import extract_amount_from_invoice, extract_numeric_from_text
from django.http import JsonResponse
import tempfile, os
...
def extract_invoice_amount_ajax(request):
    if request.method == "POST" and request.FILES.get("invoice"):
        ...
        try:
            line_text = extract_amount_from_invoice(temp_path)
            # Extract numeric from line (optional)
            numeric_amount = None
            if line_text:
        ...
        return JsonResponse({
            "amount": numeric_amount if numeric_amount else "",
            "amount_text": line_text if line_text else ""
        })
    ...
    if form.is_valid():
        try:
            with transaction.atomic():
                ...
                if expense.invoice:
                    try:
                        line_text = extract_amount_from_invoice(expense.invoice.path)
                        numeric_amount = extract_numeric_from_text(line_text)
