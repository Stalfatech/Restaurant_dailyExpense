...
import tempfile, os
from .utils.google_vision_parser import extract_invoice_amount, extract_numeric_from_text
from django.http import JsonResponse
import tempfile, os
 # we'll add extract_numeric_from_text
import os
from django.http import JsonResponse
from django.views.decorators.csrf import csrf_exempt
from .utils.google_vision_parser import extract_invoice_amount  # use correct function name
...
def extract_invoice_amount_ajax(request):
    if request.method == "POST" and request.FILES.get("invoice_file"):
        ...
        temp_path = os.path.join("temp_uploads", invoice_file.name)
        ...
        try:
            # Call your Google Vision function
            line_text, amount = extract_invoice_amount(temp_path)
            result = {"line_text": line_text, "amount": amount}

        except FileNotFoundError:
            return JsonResponse({"error": "File not found during processing."}, status=400)

        except PermissionError as e:
            # This is for Google Vision billing / permission errors
            return JsonResponse({
                "error": "Google Vision API permission denied. Check billing or API credentials.",
                "details": str(e)
            }, status=403)
    ...
    if form.is_valid():
        try:
            with transaction.atomic():
                ...
                # ✅ STEP 1: Save first to get invoice file path
                expense.save()

                # ✅ STEP 2: Extract amount from invoice
                if expense.invoice:
                    try:
                        line_text = extract_invoice_amount(expense.invoice.path)
                        numeric_amount = extract_numeric_from_text(line_text)

                        if line_text:
                            expense.amount_text = line_text
                        if numeric_amount:
                            expense.amount = numeric_amount

                        expense.save(update_fields=['amount', 'amount_text'])
