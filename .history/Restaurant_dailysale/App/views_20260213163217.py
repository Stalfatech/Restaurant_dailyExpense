...
from django.http import JsonResponse
from .utils.google_vision_parser import extract_invoice_amount
import os
...
def extract_invoice_amount_ajax(request):
    if request.method == "POST" and request.FILES.get("invoice_file"):
        ...
        try:
            ...
            # Return as JSON, convert amount to string to avoid float formatting issues
            return JsonResponse({
                "amount": str(amount) if amount else "",
                "amount_text": line_text or ""
            })
