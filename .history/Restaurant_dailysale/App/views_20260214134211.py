...
def add_daily_sale(request):
    ...
    if form.is_valid():
        ...
        # Update CashBook with cash sales
        cashbook, created = CashBook.objects.get_or_create(
            date=sale.date,
            branch=sale.branch
        )

        cashbook.cash_sales = sale.cash_sales
        cashbook.save()

        messages.success(request, "Daily sales added successfully")
        return redirect('daily_sales_dashboard')

    return render(request, 'sales/daily_sale_form.html', {'form': form})
...
def daily_sales_dashboard(request):
    ...
    # If Admin → See all branches
    if request.user.user_type == 0:
        sales = DailySale.objects.filter(date=selected_date)

    # If Manager → See only own branch
    else:
