...
def add_daily_sale(request):
    ...
    if CashBook.objects.filter(
        ...
        messages.error(request, "Day is closed. Cannot add sales.")
    ...
    if form.is_valid():
        ...
        messages.success(request, "Daily sales added successfully ✅")
        return redirect('daily_sales_dashboard')

    return render(request, 'sales/daily_sale_form.html', {
        'form': form,
        'user_type': user.user_type,
        'pagetitle': 'Add Daily Sale'
    })
...
def daily_sales_dashboard(request):
    ...
    # ✅ Safe aggregation (avoid None)
    totals = sales.aggregate(
        total_sales=Sum('total_sales'),
        total_cash=Sum('cash_sales'),
    )

    totals['total_sales'] = totals['total_sales'] or 0
    totals['total_cash'] = totals['total_cash'] or 0
...
def edit_daily_sale(request, pk):
    ...
    # 🔐 Manager can edit only their branch
    if user.user_type != 0:
...
def delete_daily_sale(request, pk):
    ...
    # 🔐 Manager can delete only their branch
    if user.user_type != 0:
        if sale.branch != user.branch:
    ...
    # ✅ Recalculate CashBook correctly
