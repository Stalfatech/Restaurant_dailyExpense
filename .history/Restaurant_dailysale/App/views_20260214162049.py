...
def dashboard(request):
    ...
    # Convert month to ISO string for JS
    monthly_sales = []
    ...
    return render(request, 'content_dash.html', {
        ...
        'expenses': expenses,
        'monthly_sales': monthly_sales,  # Already serialized
    })
...
def edit_daily_sale(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Recalculate total cash for that day + branch
    ...
    return render(request, 'sales/edit_daily_sale.html', {
        'form': form
    })
...
def history_sales(request):
    ...
    totals = sales.aggregate(
        ...
        total_delivery=Sum('delivery'),
        total_pos=Sum('pos_amount'),
        total_sales=Sum('total_sales'),   # 🔥 important
    )

    for key in totals:
        totals[key] = totals[key] or 0

    grand_total = totals['total_sales']   # 🔥 Correct
