...
def edit_expense(request, pk):
    ...
    # 🔹 Fetch expense based on user type
    if user.user_type == 0:
    ...
    if CashBook.objects.filter(
        ...
        messages.info(request, "Day is closed. Cannot edit expense.")  # light-blue alert
        return redirect('dashboard_expenses')
    ...
    if form.is_valid():
        ...
        # Assign branch and status based on user type
        if user.user_type == 0:
            branch = form.cleaned_data.get('branch')
            if not branch:
                messages.info(request, "Please select a branch.")  # light-blue alert
                return render(request, 'expenses/expense_form.html', {
                    'form': form,
                    'user_type': user.user_type
                })
            ...
            updated_expense.status = expense.status  # managers cannot approve

        updated_expense.save()

        # 🔄 Recalculate cashbooks for old and new statuses
...
def add_daily_sale(request):
    ...
    if form.is_valid():
        ...
        # 🔒 Prevent adding for closed dates
        if CashBook.objects.filter(
            date=sale.date,
            branch=sale.branch,
            is_closed=True
        ).exists():
            messages.info(request, "Cannot add sales for a closed day.")  # light-blue
            return redirect('daily_sales_dashboard')
        ...
        cashbook.cash_sales = total_cash
        cashbook.closing_balance = cashbook.opening_balance + cashbook.cash_sales - cashbook.expenses
        cashbook.save()
...
def edit_daily_sale(request, pk):
    ...
    # 🔒 Prevent editing if day closed
    if CashBook.objects.filter(
        date=sale.date,
        branch=sale.branch,
        is_closed=True
    ).exists():
        messages.info(request, "Day is closed. Cannot edit sales.")
        return redirect('daily_sales_dashboard')
    ...
    if form.is_valid():
        ...
        # 🔄 Recalculate OLD cashbook
        update_cashbook(old_branch, old_date)

        # 🔄 Recalculate NEW cashbook
        update_cashbook(updated_sale.branch, updated_sale.date)
