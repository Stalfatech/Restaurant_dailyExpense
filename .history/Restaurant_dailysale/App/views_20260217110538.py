...
def add_daily_sale(request):
    ...
    item_formset = DailySaleItemFormSet(
        request.POST or None,
        prefix='items'  # MUST match template JS
    )

    delivery_formset = DeliveryFormSet(
        request.POST or None,
        prefix='deliveries'  # MUST match template JS
    )
...
def edit_daily_sale(request, pk):
    ...
    # SECURITY CHECK
    # ======================
    if user.user_type != 0:  # Not admin
        if not user.branch or sale.branch != user.branch:
    ...
    item_formset = DailySaleItemFormSet(
        request.POST or None,
        instance=sale,
        prefix='items'  # MUST MATCH JS
    )

    delivery_formset = DeliveryFormSet(
        request.POST or None,
        instance=sale,
        prefix='deliveries'  # MUST MATCH JS
    )
...
def add_delivery_platform(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            return redirect('add_delivery_platform')
