...
def dashboard(request):
    ...
    if user.user_type == 0:  # Admin sees all branches
        ...
        for branch in branches:
            ...
            branch_salaries.append({
                'branch': branch,
                'total_salary': total_salary
            })

    elif user.user_type == 1:  # Manager sees only their branch
        branch = user.branch
        total_salary = Salary.objects.filter(
            staff__branch=branch
        ).aggregate(total_paid=Sum('paid_amount'))['total_paid'] or 0
    ...
    context = {
        ...
        'branch_cashbooks': branch_cashbooks,  # existing balances table
        'branch_salaries': branch_salaries,    # new total salary table
    }
