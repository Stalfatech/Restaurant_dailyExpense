...
def add_salary(request):
    ...
    if request.method == "POST":
        ...
        advance_input = Decimal(request.POST.get('salary_advance') or 0)
...
def get_previous_salary_data(request):
    ...
    if salary_row:
        ...
        previous_next_month_advance = salary_row.next_month_advance_amount or Decimal('0')

    return JsonResponse({
        'previous_balance': str(previous_balance),
        'previous_next_month_advance': str(previous_next_month_advance)
    })
...
def edit_salary(request, pk):
    ...
    # -------------------------------
    # total advance for this salary month
    # -------------------------------
...
def communication_settings(request):
    ...
    return render(request, 'Admin/communication_settings.html', {'form': form})
...
from .utils import generate_pdf, send_email_report, send_whatsapp_report


def send_report(request, report_type):
    user = request.user

    # ===== Get filters from URL (same as report page) =====
    order_id = request.GET.get('order_id')
    staff_id = request.GET.get('staff')
    selected_date = request.GET.get('date')

    # ===== Staff visibility (same logic as your report) =====
    if user.user_type == 0:
        staff_list = Staff.objects.filter(role='Delivery', status='Active')
    else:
        staff_list = Staff.objects.filter(
    ...
    # ===== Generate PDF (filtered data) =====
    pdf = generate_pdf("sales/delivery_report.html", context)
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            message = f"Delivery Report Total: {grand_total}"
            return send_whatsapp_report(number, message)

    # ===== Show export page first =====
    return render(request, "reports/export_page.html", {
        'report_type': report_type
    })