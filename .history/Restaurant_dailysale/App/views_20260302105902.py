...
def reports_list(request):
    reports = [
        ...
        {"name": "Expense History", "url": "history_expenses"},
         {"name": "Daily Salary Report", "url": "admin_daily_salary_list"},
    ]
...
def export_report_page(request, report_type):
    ...
    elif report_type == "staffdaily_salary":
        action_url = reverse('send_adminstaff_addaily_salary_report')
...
def send_adminstaff_daily_salary_report(request):
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            return redirect(f"{reverse('admindailysalary_list')}?{query_params}")
