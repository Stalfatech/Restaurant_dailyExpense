...
def send_adminstaff_monthly_salary_report(request):
    ...
    # ===== Generate PDF =====
    pdf = generate_pdf("Admin/adminmonthly_salary_export.html", context)
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            return redirect(f"{reverse('admin_staff_monthly_salary_report')}?{query_params}")
    ...
    return render(request, "reports/export_page.html", {
        "report_type": report_type,
        "query_params": query_params,
        "action_url": reverse("send_adminmonthly_salary_report")
    })