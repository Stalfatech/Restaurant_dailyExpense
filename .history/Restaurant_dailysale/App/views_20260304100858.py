...
if user.user_type == 0:  # Only Admin sees manager salaries
    ...
    managers = Manager.objects.select_related('user', 'branch').all().order_by('branch__name', 'user__username')

    for manager in managers:

        total_paid = ManagerSalary.objects.filter(
            manager=manager
        ).aggregate(total_paid=Sum('paid_amount'))['total_paid'] or 0

        total_salary_amount = ManagerSalary.objects.filter(
            manager=manager
        ).aggregate(total_salary=Sum('salary_amount'))['total_salary'] or 0

        total_balance = ManagerSalary.objects.filter(
            manager=manager
        ).aggregate(total_balance=Sum('balance_salary'))['total_balance'] or 0
