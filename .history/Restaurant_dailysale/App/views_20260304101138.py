...
def dashboard(request):
    ...
    context = {
        ...
        'branch_cashbooks': branch_cashbooks,# ---------------- Manager Salaries Summary -----------------
          manager_salaries = []

if user.user_type == 0:  # Only Admin sees manager salaries

    managers = Manager.objects.select_related('user').all().order_by('user__username')

    for manager in managers:

        salary_qs = ManagerSalary.objects.filter(manager=manager)

        total_paid = salary_qs.aggregate(
            total_paid=Sum('paid_amount')
        )['total_paid'] or 0

        total_salary_amount = salary_qs.aggregate(
            total_salary=Sum('salary_amount')
        )['total_salary'] or 0

        total_balance = salary_qs.aggregate(
            total_balance=Sum('balance_salary')
        )['total_balance'] or 0

        # Get branch from latest salary entry (if exists)
        latest_salary = salary_qs.order_by('-id').first()
        branch = latest_salary.branch if latest_salary else None

        manager_salaries.append({
            'manager': manager,
            'branch': branch,
            'total_paid': total_paid,
            'balance_salary': total_balance
        })
...
@login_required
def send_staff_daily_salary_report(request):

    report_type = "daily_salary"
    ...
    # Generate PDF
    pdf = generate_pdf(
        "Manager/daily_salary_export.html",
        context
    )
    ...
    return render(request, "reports/export_page.html", {
        ...
        "action_url": reverse("send_staff_daily_salary_report")
    })
...
def send_monthly_salary_report(request):
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            messages.success(request, "Monthly salary report sent via Email.")
...
def admindailysalary_list(request):
    ...
    total_balance = sum(s.balance_salary for s in salaries)
...
def manager_monthly_report(request):
    ...
    return render(
        request,
        'Admin/manager_monthlyreport.html',
        context
    )
...
@admin_or_manager_required
def send_adminstaff_daily_salary_report(request):
    report_type = "staff_daily_salary"
    query_params = request.GET.urlencode()
    ...
    # ===== Generate PDF =====
    pdf = generate_pdf("Admin/daily_adminstaff_salary_export.html", context)
...
@admin_or_manager_required
def send_adminstaff_monthly_salary_report(request):
    report_type = "staff_monthly_salary"
    query_params = request.GET.urlencode()
    ...
    # ===== Generate PDF =====
    pdf = generate_pdf("Admin/monthly_adminstaff_salary_export.html", context)
...
@login_required
def send_manager_monthly_report(request):

    report_type = "manager_monthly_salary"
    query_params = request.GET.urlencode()
    ...
    total_paid = totals.get("total_paid") or Decimal("0.00")
    total_advance = totals.get("total_advance") or Decimal("0.00")
    total_next_advance = totals.get("total_next_advance") or Decimal("0.00")
    total_balance = totals.get("total_balance") or Decimal("0.00")
    ...
    # Generate PDF
    pdf = generate_pdf(
        "Admin/manager_monthly_salary_export.html",
        context
    )
    ...
    if request.method == "POST":
        ...
        # WhatsApp
            ...
            message = (
                f"Manager Monthly Salary Report\n"
                f"Total Paid: {total_paid}\n"
                f"Download: {pdf_link}"
            )
