...
def dashboard(request):
    ...
    if user.user_type == 0:  # Admin only
    ...
    for manager in managers:
        ...
        total_data = salary_qs.aggregate(
            total_paid=Sum('paid_amount'),
            total_balance=Sum('balance_salary')
        )

        total_paid = total_data['total_paid'] or 0
        total_balance = total_data['total_balance'] or 0

        # Get branch from latest salary entry
        latest_salary = salary_qs.order_by('-salary_year', '-id').first()
