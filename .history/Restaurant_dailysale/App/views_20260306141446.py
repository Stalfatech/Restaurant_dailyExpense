...
def add_manager(request):
    ...
    if request.method == "POST":
        ...
        if not all([name, phone, dob, address, joining_date, email, password, branch_id]):
            notify(request, "All mandatory fields are required")
        ...
        # Email exists
        if User.objects.filter(email=email).exists():
            notify.error(request, "Email already exists")
            return render(request, 'Admin/add_manager.html', context)
        ...
        try:
            ...
            notify.success(request, "Manager added successfully")
            return redirect('manager_view')

        except Exception as e:
            notify.error(request, f"Error: {str(e)}")
            return render(request, 'Admin/add_manager.html', context)
...
def manager_delete(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            notify.success(request, "Manager deleted successfully")
        except:
            notify.error(request, "Unable to delete manager")
        return redirect('manager_view')
...
def manager_edit(request, id):
    ...
    if request.method == "POST":
        ...
        if not all([name, email, phone, branch_id]):
            notify.error(request, "All mandatory fields are required")
