...
def notifications_list(request):
    ...
    if user.user_type == 1:  # Manager
        notifications = notifications.filter(branch=user.branch)
    # Admin (user_type == 0) sees all, no filter needed
...
def delete_all_notifications(request):
    ...
    if request.method == "POST":
        ...
        elif user.user_type == 1:  # Manager deletes only their branch
