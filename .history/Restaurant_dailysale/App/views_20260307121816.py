...
def send_report(request, report_type):
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            error=send_email_report(email, pdf, f"{report_type}_report.pdf")
            if error:
                messages.info(request, f"Failed to send email: {error}")
                return redirect(f"{reverse('export_report_page', args=[report_type])}?{query_params}")
            notify(request, "Email sent successfully")
...
def send_adminstaff_monthly_salary_report(request):
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            return redirect(f"{reverse('admin_monthly_salary_list')}?{query_params}")
