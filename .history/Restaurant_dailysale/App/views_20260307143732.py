...
def edit_daily_sale(request, pk):
    ...
    delivery_formset = DeliveryFormSet(
        ...
        prefix='deliveries',
        instance=sale,
        form_kwargs={'user': request.user}  # ✅ SAME AS ADD VIEW
    )
...
def send_report(request, report_type):
    ...
    deliveries = DeliverySale.objects.select_related('staff', 'sale','sale__branch').filter(
    ...
    email,
    "Delivery Report",
    f"Total Delivery Collection: {grand_total}",
    pdf
)
            if error:
                messages.info(request,error)
                return redirect(f"{reverse('export_report_page', args=[report_type])}?{query_params}")
            notify(request, "Delivery report sent via email successfully")
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            if error:
                messages.info(request, error)
                return redirect(f"{reverse('export_report_page', args=[report_type])}?{query_params}")
            notify(request, "Expense report sent successfully via Email.")
    ...
    if request.method == "POST":
        ...
        if send_via == "email":
            ...
            if error:
                messages.info(request, error)
                return redirect(f"{reverse('export_report_page', args=[report_type])}?{query_params}")
            notify(request, "Staff monthly salary report sent via Email.")
            return redirect(f"{reverse('admin_salary_list')}?{query_params}")
